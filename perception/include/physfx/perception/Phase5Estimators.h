 ...
 */
class PHYSFX_API DepthAnythingEstimator final : public IDepthEstimator {
 public:
  core::DepthMap estimate(const core::Frame& frame) override;
};

/** @brief 天空分割的无权重回退，实现顶部先验与颜色/梯度判别。 */
class PHYSFX_API SkySegmenter final : public ISkySegmenter {
 public:
  core::OcclusionMask segment(const core::Frame& frame) override;
};

/** @brief 基于亮度梯度的主光方向估计，输出稳定的屏幕空间方向。 */
class PHYSFX_API HeuristicLightEstimator final : public ILightEstimator {
 public:
