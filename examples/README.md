...
```powershell
ffmpeg -f lavfi -i testsrc2=size=320x240:rate=30 -t 1 -pix_fmt yuv420p build/demo-input.mp4
build\windows\apps\cli\physfx.exe run --config examples\particle_occlusion_demo.json
```

输出为 `build/particle-occlusion-demo.mp4`。示例默认使用提示点颜色连通分割与质心
跟踪，因此无需模型即可验证真实计算的遮挡链路；准备好经许可审查的 SAM/XMem ONNX 权重后，将
